...
func process_tick():
	...
	process_ticked.emit()
	if not is_idle: input_direction = ((nav_agent.get_next_path_position() - body.global_position) * Vector3(1.0, 0.0, 1.0)).normalized() if nav_agent.distance_to_target() > 0.3 else Vector3.ZERO
	else: input_direction = Vector3.ZERO
