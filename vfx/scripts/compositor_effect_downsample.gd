...
class_name CompositorEffectDownsample


@export var downsamples : float = 2
...
func _init() -> void:
	var shader_file: RDShaderFile = preload("res://vfx/shaders/downsample.glsl")
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	var parameter_data := PackedFloat32Array([size.x, size.y, 0.0, 0.0, downsamples]).to_byte_array()
